...
# Create save paths
lock_path = "./Plots/dyn_lock/mlp_zeroshot"
mkdir(lock_path)
for i in 1:3
...
function plot_scenarios(data, pred_base, pred_lock, group_idx, target_groups; denorm=false)
    ...
    for i in eachindex(all_states)
        ...
        savefig(p, "./Plots/dyn_lock/mlp_zeroshot/gr$(group_idx)/lock_$state.png")
...
# Initialize arrays to hold the total number of samples across the ensemble
pred_base_samples = zeros(Float32, n_total_samples, n_times, n_nodes)
preds_lock_samples = zeros(Float32, n_total_samples, num_groups, n_times, n_nodes)

splines_base = build_splines(X_norm, Int[], lags)  # no lockdown in baseline

# Prepare Model architecture logic (Use the latent dim from the first model as reference)
...
# Simulate scenarios across the ensemble and latent draws
println("Simulating scenarios with $n_total_samples total samples ($n_models models × $n_latent_samples latents)...")

sample_idx = 1
for m_idx in 1:n_models
    ps_current = ensemble_ps[m_idx]
    
    # Calculate latent distribution metrics for the current model
    ...
    for s in 1:n_latent_samples
        ...
        new_latents_test = (randn(rng, Float32, latent_dim, length(test_states)) .* σ_lat) .+ μ_lat
        ...
        pred_base_samples[sample_idx, :, :] .= pred_base_model[1, :, :]
        ...
        preds_lock_samples[sample_idx, :, :, :] .= preds_lock_model
        
        global sample_idx += 1
    end
end

# Compute Median across the total sample dimension (dim=1)
...
for g in 1:num_groups
    ...
    clims = [(-0.11, 0.0), (-0.18, 0.0), (-0.18, 0.0)]
