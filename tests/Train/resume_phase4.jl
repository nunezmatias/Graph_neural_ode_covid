# resume_phase4.jl
#
# Extends Cosine Annealing optimization from the definitive Phase 3 checkpoint
# to see if the network can reach an even deeper global minimum.
# Architecture: 40 states, 400 days, GNN-ODE. Width 64.
...
println("=== PHASE 4: EXTENDED COSINE ANNEALING OVERHAUL ===\n")
...
gnn = ExplicitGNN(nin_tot, 64, 1, 0.0) # Width 64, No dropout in Phase 4
_, st_gnn = Lux.setup(rng, gnn)

model_state = st_gnn

# ─── 3. Load Phase 3 Checkpoint ──────────────────────────────────────────────
ckpt_path = "Resultados/test-8BIS/checkpoints/params_40s_phase3.jld2"
println("Loading Phase 3 definitive checkpoint: params_40s_phase3.jld2")
...
scheduler = CosineAnnealing(2e-5, 5e-7, epochs_per_cycle)
...
println("\nStarting Phase 4 Extended Global Training (MAE Loss, Cosine Annealing)")
...
for c in 1:cycles
    ...
    for ep in 1:epochs_per_cycle
        ...
        # Initial forward pass to compute MAE
        ...
        if mae < best_mae
            global best_mae = mae
            jldsave("Resultados/test-8BIS/checkpoints/params_40s_phase4_ext.jld2"; ps_final=current_ps)
            println("  > Extended Checkpoint saved (best MAE = $(round(mae, digits=5)))")

            p = plot(mae_history, label="MAE", xlabel="Epoch", ylabel="Loss", title="Phase 4 Extended Training")
            savefig(p, "Resultados/test-8BIS/plots/phase4_ext_loss.png")
            println("  > Plot saved")
        end
    end
end

println("\nPhase 4 Extended Training Complete!")
