...
function ExplicitGNN(nin, nhidden, nout, drop_p)
    return ExplicitGNN(
        WeightedGraphLayer(nin, nhidden; act=tanh),
        FrozenDropout(drop_p),
        WeightedGraphLayer(nhidden, nhidden; act=tanh),
        FrozenDropout(drop_p),
        WeightedGraphLayer(nhidden, nhidden; act=tanh),
        FrozenDropout(drop_p),
        WeightedGraphLayer(nhidden, nout)
    )
end
...
function (layer::ExplicitGNN)(A::AbstractMatrix, x::AbstractMatrix, ps, st)
    x, st_c1 = layer.layer1(x, A, ps.layer1, st.layer1)
    ...
    x, st_c4 = layer.layer4(x, A, ps.layer4, st.layer4)
...
function train_model(X_norm::Array{Float32, 3}, A::Matrix{Float32}, tsteps::Vector{Float32}, spls::Matrix, save_path::String; seed::Int=123)
    ...
    # SAVE
    # ==========================================================================
    param_file = joinpath(save_path, "params_custom.jld2")
    loss_file = joinpath(save_path, "lossHist_custom.jld2")
