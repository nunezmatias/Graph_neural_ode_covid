...
const CONFIG = (
    ...
    # --- Spatial Diffusion Regularization Hyperparameters ---
    reg_lambda = 5f-3,   # Penalty strength multiplier (tune as needed)
    reg_ratio  = 0.3f0   # Target minimum ratio of spatial-to-local weights (||W_neigh|| / ||W_self||)
)
...
# Smoothly differentiable spatial penalty function
function diffusion_regularization_penalty(ps_gnn; min_ratio=CONFIG.reg_ratio)
    penalty = 0.0f0
    for layer_name in (:layer1, :layer2, :layer3, :layer4)
        layer_ps = ps_gnn[layer_name]
        if hasproperty(layer_ps, :W_self) && hasproperty(layer_ps, :W_neigh)
            norm_self = sqrt(sum(abs2, layer_ps.W_self) + 1e-8)
            norm_neigh = sqrt(sum(abs2, layer_ps.W_neigh) + 1e-8)
            
            # Using max() instead of branching "if" statement for clean Zygote differentiation
            penalty += max(0.0f0, min_ratio * norm_self - norm_neigh)
        end
    end
    return penalty
end

function report_layer_norms(ps_gnn)
    stats = NamedTuple[]
    for layer_name in (:layer1, :layer2, :layer3, :layer4)
        layer_ps = ps_gnn[layer_name]
        if hasproperty(layer_ps, :W_self) && hasproperty(layer_ps, :W_neigh)
            ns = norm(layer_ps.W_self)
            nn = norm(layer_ps.W_neigh)
            push!(stats, (layer=layer_name, norm_self=ns, norm_neigh=nn, ratio=nn/(ns+1f-8)))
        end
    end
    return stats
end

function print_layer_norms(stats, prefix="")
    for s in stats
        println("   $prefix $(s.layer): ||W_self||=$(round(s.norm_self,digits=3)) " *
                "||W_neigh||=$(round(s.norm_neigh,digits=3)) ratio=$(round(s.ratio,digits=3))")
    end
end
...
function train_model(X_norm::Array{Float32, 3}, A::Matrix{Float32}, tsteps::Vector{Float32}, spls::Matrix, save_path::String; seed::Int=123)
    ...
    function loss_function(p, t_bounds, gnn_obj, st_obj) 
        ...
        mae_loss = mean(abs, target .- pred) 
        
        # Auxiliary objective: Force spatial integration using Decoupled Weight Norms
        reg_penalty = diffusion_regularization_penalty(p.gnn)
        total_loss = mae_loss + CONFIG.reg_lambda * reg_penalty
        
        return total_loss, st_obj, NamedTuple() 
    end
    ...
    # ==========================================================================
    norm_log = NamedTuple[]  # will hold (phase, epoch, layer, norm_self, norm_neigh, ratio)
    println("\n--- PHASE 1: Temporal Chunking ---")
    ...
    for i in 1:CONFIG.p1_epochs
        ...
        if i % 10 == 0
            println("Ph1 $i/$(CONFIG.p1_epochs) | Loss: $(round(l, digits=5)) | $(dt)s")
        end

        if i % 50 == 0
            stats = report_layer_norms(tstate1.parameters.gnn)
            for s in stats
                push!(norm_log, (phase="P1", epoch=i, layer=s.layer,
                              norm_self=s.norm_self, norm_neigh=s.norm_neigh, ratio=s.ratio))
            end
            print_layer_norms(stats, "[P1 ep$i]")
        end
    ...
    for i in 1:CONFIG.p2_epochs
        ...
        if i % 10 == 0
            println("Ph2 $i/$(CONFIG.p2_epochs) | Loss: $(round(l, digits=5)) | $(dt)s")
        end

        if i % 50 == 0
            stats = report_layer_norms(tstate1.parameters.gnn)
            for s in stats
                push!(norm_log, (phase="P2", epoch=i, layer=s.layer,
                              norm_self=s.norm_self, norm_neigh=s.norm_neigh, ratio=s.ratio))
            end
            print_layer_norms(stats, "[P2 ep$i]")
        end
    ...
    for i in 1:p3_epochs
        ...
        if i % 10 == 0
            println("Ph3 $i/$(p3_epochs) | Loss: $(round(l, digits=5)) | $(dt)s")
        end

        if i % 50 == 0
            stats = report_layer_norms(tstate1.parameters.gnn)
            for s in stats
                push!(norm_log, (phase="P3", epoch=i, layer=s.layer,
                              norm_self=s.norm_self, norm_neigh=s.norm_neigh, ratio=s.ratio))
            end
            print_layer_norms(stats, "[P3 ep$i]")
        end
    ...
    for i in 1:p4_epochs
        ...
        if i % 10 == 0
            println("Ph4 $i/$(p4_epochs) | Loss: $(round(l, digits=5)) | $(dt)s")
        end

        if i % 50 == 0
            stats = report_layer_norms(tstate1.parameters.gnn)
            for s in stats
                push!(norm_log, (phase="P4", epoch=i, layer=s.layer,
                              norm_self=s.norm_self, norm_neigh=s.norm_neigh, ratio=s.ratio))
            end
            print_layer_norms(stats, "[P4 ep$i]")
        end
    ...
    for i in 1:CONFIG.p5_epochs
        ...
        if i % 50 == 0
            stats = report_layer_norms(tstate1.parameters.gnn)
            for s in stats
                push!(norm_log, (phase="P5", epoch=i, layer=s.layer,
                              norm_self=s.norm_self, norm_neigh=s.norm_neigh, ratio=s.ratio))
            end
            print_layer_norms(stats, "[P5 ep$i]")
        end
    ...
    # SAVE
    # ==========================================================================
    param_file = joinpath(save_path, "params.jld2")
    loss_file = joinpath(save_path, "lossHist.jld2")
    norm_file = joinpath(save_path, "normLog.jld2")
