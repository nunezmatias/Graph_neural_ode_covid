...
function ExplicitGNN(nin, nhidden, nout, drop_p)
    return ExplicitGNN(
        GNNLux.GCNConv(nin => nhidden, tanh; use_edge_weight=true, add_self_loops=true),
        FrozenDropout(drop_p),
        GNNLux.GCNConv(nhidden => nhidden, tanh; use_edge_weight=true, add_self_loops=true),
        FrozenDropout(drop_p),
        GNNLux.GCNConv(nhidden => nhidden, tanh; use_edge_weight=true, add_self_loops=true),
        FrozenDropout(drop_p),
	GNNLux.GCNConv(nhidden => nout; use_edge_weight=true, add_self_loops=true)
    )
end
...
function (layer::ExplicitGNN)(g::GNNGraph, x::AbstractMatrix, ps, st)
    
    x, st_c1 = layer.layer1(g, x, ps.layer1, st.layer1)
    x, st_d1 = layer.drop1(x, ps.drop1, st.drop1)
    
    x, st_c2 = layer.layer2(g, x, ps.layer2, st.layer2)
    x, st_d2 = layer.drop2(x, ps.drop2, st.drop2)
    
    x, st_c3 = layer.layer3(g, x, ps.layer3, st.layer3)
    x, st_d3 = layer.drop3(x, ps.drop3, st.drop3)
    
    x, st_c4 = layer.layer4(g, x, ps.layer4, st.layer4)
    
    new_st = (layer1=st_c1, drop1=st_d1, layer2=st_c2, drop2=st_d2, layer3=st_c3, drop3=st_d3, layer4=st_c4)
    return x, new_st
end
...
# ==============================================================================
function train_model(X_norm::Array{Float32, 3}, g::Matrix{Float32}, tsteps::Vector{Float32}, spls::Matrix, save_path::String; seed::Int=123)
    strt_time = time()
    ...
    # SAVE
    # ==========================================================================
    param_file = joinpath(save_path, "params_GCN.jld2")
    loss_file = joinpath(save_path, "lossHist_GCN.jld2")
