...
println(stdout, "=== Test 7: ISOLATED NODES ===")
flush(stdout)

function create_isolated_graph(g_orig)
    n_nodes = g_orig.num_nodes
    A_iso = zeros(Float64, n_nodes, n_nodes)
    for i in 1:n_nodes
        A_iso[i, i] = 1.0
    end
    return GNNGraph(A_iso)
end
g = create_isolated_graph(g)
println(stdout, "Graph modified: Isolated nodes (diagonal only)")
flush(stdout)
...
function plot_callback(state_data, pred_data, epoch, loss_history)
    mkpath("Resultados/test-7/plots/training_isolated")
    ...
    for node in 1:6
        ...
        savefig(plt, "Resultados/test-7/plots/training_isolated/epoch_$(epoch)_node_$(node).png")
    end
    ...
    savefig(plt_loss, "Resultados/test-7/plots/training_isolated/epoch_$(epoch)_loss.png")
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    @save "Resultados/test-7/checkpoints/params_isolated.jld2" ps_trained
    ...
    for node in 1:6
        ...
        savefig(plt, "Resultados/test-7/plots/training_isolated/final_state_$(states[node]).png")
    end
end
