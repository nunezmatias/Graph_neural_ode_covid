...
# TEST 7: RANDOM GRAPH
println(stdout, "=== Test 7: RANDOM GRAPH ===")
flush(stdout)
...
println(stdout, "Graph modified: Random topology (same density)")
flush(stdout)
...
function plot_callback(state_data, pred_data, epoch, loss_history)
    mkpath("Resultados/test-7/plots/training_random")
    ...
    for node in 1:6
        ...
        savefig(plt, "Resultados/test-7/plots/training_random/epoch_$(epoch)_node_$(node).png")
    end
    ...
    savefig(plt_loss, "Resultados/test-7/plots/training_random/epoch_$(epoch)_loss.png")
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    @save "Resultados/test-7/checkpoints/params_random.jld2" ps_trained
    ...
    for node in 1:6
        ...
        savefig(plt, "Resultados/test-7/plots/training_random/final_state_$(states[node]).png")
    end
end
