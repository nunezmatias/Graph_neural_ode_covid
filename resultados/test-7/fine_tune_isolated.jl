...
# TOPOLOGY MODIFICATION: ISOLATED
A_iso = Matrix(1.0 * I, g.num_nodes, g.num_nodes)
g = GNNGraph(A_iso)
...
# 3. LOAD CHECKPOINT
checkpoint_path = "Resultados/test-7/checkpoints/params_isolated.jld2"
...
function train_fine_tune!(model, ps, st, opt, tsteps, X_data)
    ...
    for epoch in 1:1000
        ...
        flush(stdout)
        if epoch % 50 == 0
            @save "Resultados/test-7/checkpoints/params_isolated_finetuned.jld2" ps_trained = tstate.parameters
        end
    end
...
@save "Resultados/test-7/checkpoints/params_isolated_finetuned.jld2" ps_trained = ps_final
println("Fine-tuning Isolated Complete.")
