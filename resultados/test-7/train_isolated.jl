# Test 7: Train ISOLATED NODES topology
...
Random.seed!(rng, 43)  # Different seed

TOPOLOGY = :isolated
println("=== Training: ISOLATED NODES ===\n")
...
# Create ISOLATED graph (diagonal only)
A_iso = zeros(Float64, n_nodes, n_nodes)
for i in 1:n_nodes
    A_iso[i, i] = 1.0
end
g_train = GNNGraph(A_iso)
...
@save "Resultados/test-7/checkpoints/params_isolated.jld2" ps_full
CSV.write("Resultados/test-7/checkpoints/loss_isolated.csv", DataFrame(epoch=1:length(loss_history), loss=loss_history))
println("\n✅ ISOLATED NODES COMPLETE: Final Loss = $(round(loss_history[end], digits=6))")
