...
# normalize log
# μ = mean(X_true, dims=(2,3))
# σ = std(X_true, dims=(2,3))
X_norm = log.(X_true .+ 1)
u0 = X_norm[1, 1, :]

# time
tsteps = collect(0.0:1.0:size(X_true, 2)-1)
...
covariate_splines = [CubicSpline(tsteps, @view X_norm[v, :, n]) for v in 2:size(X_norm, 1), n in 1:size(X_norm, 3)]
